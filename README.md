...
Pricing with a single historical volatility therefore produces a large, structured error — up to **320%** of an option's market price in this dataset. The project quantifies that error and reduces it by estimating the volatility surface from market data instead of assuming a constant.
...
4. Fits the IV surface over (moneyness, maturity) with two models: polynomial regression with a Ridge penalty, and Random Forest
...
Random Forest wins clearly on calls but only marginally on puts. Neither model dominates: the polynomial surface is smoother and generalises better at scale, while Random Forest tracks observed points more closely but turns stepped where data is sparse. **Model selection turns out to be data-dependent**, which is the more interesting finding than either RMSE on its own.
...
- **Underlying:** SPDR S&P 500 ETF Trust (SPY) — high liquidity, no idiosyncratic single-stock effects
...
Market data is not committed to the repo; the notebook fetches it live and can save a snapshot locally.
...
Run during US market hours — outside them many contracts quote zero bid/ask and get dropped by the liquidity filters. Results will differ from the report, which uses a snapshot from 7 August 2026.

## Known limitations

Covered in full in the report. The main ones:

- Single-day snapshot on a single underlying — no evidence the surface generalises across time or assets
- Train-test split is random, not temporal, so out-of-sample performance under different market conditions is untested
